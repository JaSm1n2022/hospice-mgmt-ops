...
import {
  ...
  setCreateProductSucceed,
  setDeleteProductFailure,
  setDeleteProductSucceed,
  setFetchProductFailure,
  setFetchProductSucceed,
  setUpdateProductFailure,
  setUpdateProductSucceed,
...
function* categoryQuery(filter) {
  try {
    ...
    let { data, error, status } = yield supabaseClient
      ...
      .select()
      .eq("companyId", filter.payload.companyId)
      .eq("category", filter.payload.category);
...
function* createProduct(rqst) {
  try {
    ...
    if (error) {
      ...
      yield put(
        setCreateProductFailure(`[create Product] : ${error.toString()}`)
      );
      throw error;
    }
...
function* updateProduct(rqst) {
  try {
    ...
    if (error) {
      ...
      yield put(
        setUpdateProductFailure(`[update Product] : ${error.toString()}`)
      );
      throw error;
    }
...
function* deleteProduct(rqst) {
  try {
    ...
    if (error) {
      ...
      yield put(
        setDeleteProductFailure(`[delete Product] : ${error.toString()}`)
      );
      throw error;
    }
