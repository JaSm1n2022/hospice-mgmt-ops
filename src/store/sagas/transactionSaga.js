...
import {
  TRANSACTION_ACTIONS,
  setCreateTransactionFailure,
  setCreateTransactionSucceed,
  setDeleteTransactionFailure,
  setDeleteTransactionSucceed,
  setFetchTransactionFailure,
  setFetchTransactionSucceed,
  setUpdateTransactionFailure,
  setUpdateTransactionSucceed,
...
function* listTransaction(filter) {
  try {
    ...
    let { data, error, status } = yield supabaseClient
      ...
      .gte("ordered_at", `${filter.payload.from} 00:00`)
      .lt("ordered_at", `${filter.payload.to} 23:59`);
...
function* CreateTransaction(rqst) {
  try {
    ...
    if (error) {
      ...
      yield put(
        setCreateTransactionFailure(
          `[create Transaction] : ${error.toString()}`
        )
      );
      throw error;
    }
    ...
    console.log(`[create Transaction] : ${error.toString()}`);
    yield put(
      setCreateTransactionFailure(`[create Transaction] : ${error.toString()}`)
    );
  }
}
...
function* UpdateTransaction(rqst) {
  try {
    ...
    if (error) {
      ...
      yield put(
        setUpdateTransactionFailure(
          `[update Transaction] : ${error.toString()}`
        )
      );
      throw error;
    }
    ...
    console.log(`[update Transaction] : ${error.toString()}`);
    yield put(
      setUpdateTransactionFailure(`[update Transaction] : ${error.toString()}`)
    );
  }
}
...
function* DeleteTransaction(rqst) {
  try {
    ...
    if (error) {
      ...
      yield put(
        setDeleteTransactionFailure(
          `[delete Transaction] : ${error.toString()}`
        )
      );
      throw error;
    }
    ...
    console.log(`[delete Transaction] : ${error.toString()}`);
    yield put(
      setDeleteTransactionFailure(`[delete Transaction] : ${error.toString()}`)
    );
  }
}
...
function* transactionsagaWatcher<T>(): Iterable<T> {
  yield takeEvery(
    TRANSACTION_ACTIONS.ATTEMPT_TO_FETCH_TRANSACTION,
    listTransaction
  );
  yield takeLatest(
    TRANSACTION_ACTIONS.ATTEMPT_TO_CREATE_TRANSACTION,
    CreateTransaction
  );
  yield takeLatest(
    TRANSACTION_ACTIONS.ATTEMPT_TO_UPDATE_TRANSACTION,
    UpdateTransaction
  );
  yield takeLatest(
    TRANSACTION_ACTIONS.ATTEMPT_TO_DELETE_TRANSACTION,
    DeleteTransaction
  );
}
