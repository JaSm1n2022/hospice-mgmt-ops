...
import {
  setCreateTemplateFailure,
  setCreateTemplateSucceed,
  setDeleteTemplateFailure,
  setDeleteTemplateSucceed,
  setFetchTemplateFailure,
  setFetchTemplateSucceed,
  setUpdateTemplateFailure,
  setUpdateTemplateSucceed,
  TEMPLATE_ACTIONS,
...
function* createTemplate(rqst) {
  try {
    ...
    if (error) {
      yield put(
        setCreateTemplateFailure(`[create Template] : ${error.toString()}`)
      );
      throw error;
    }
...
function* updateTemplate(rqst) {
  try {
    ...
    if (error) {
      yield put(
        setUpdateTemplateFailure(`[update Template] : ${error.toString()}`)
      );
      throw error;
    }
...
function* deleteTemplate(rqst) {
  try {
    ...
    if (error) {
      yield put(
        setDeleteTemplateFailure(`[delete Template] : ${error.toString()}`)
      );
      throw error;
    }
