...
import {
  EMPLOYEE_ACTIONS,
  setCreateEmployeeFailure,
  setCreateEmployeeSucceed,
  setDeleteEmployeeFailure,
  setDeleteEmployeeSucceed,
  setFetchEmployeeFailure,
  setFetchEmployeeSucceed,
  setUpdateEmployeeFailure,
  setUpdateEmployeeSucceed,
...
function* createEmployee(rqst) {
  try {
    ...
    if (error) {
      ...
      yield put(
        setCreateEmployeeFailure(`[create Employee] : ${error.toString()}`)
      );
      throw error;
    }
...
function* updateEmployee(rqst) {
  try {
    ...
    if (error) {
      ...
      yield put(
        setUpdateEmployeeFailure(`[update Employee] : ${error.toString()}`)
      );
      throw error;
    }
...
function* deleteEmployee(rqst) {
  try {
    ...
    if (error) {
      ...
      yield put(
        setDeleteEmployeeFailure(`[delete Employee] : ${error.toString()}`)
      );
      throw error;
    }
