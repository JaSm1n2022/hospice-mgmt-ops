...
class TransactionHandler {
  ...
  static mapData(items) {
    items.forEach((item) => {
      ...
      item.payment_transaction_at = moment(item.payment_transaction_at).format(
        "YYYY-MM-DD"
      );
      item.expected_delivery_at = moment(item.expected_delivery_at).format(
        "YYYY-MM-DD"
      );
    });
