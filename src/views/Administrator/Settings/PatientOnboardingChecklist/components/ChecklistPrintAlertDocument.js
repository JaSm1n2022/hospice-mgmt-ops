...
const collectPatientAlerts = (patient) => {
  ...
  // Check Compliance items - SEPARATED for red highlighting
  if (patient.compliance) {
    ["hopeAdmission", "hopeHuv1", "hopeHuv2", "hopeDischarge"].forEach((key) => {
      const item = patient.compliance[key];
      if (!item || !item.value || (item.value !== "Y" && item.value !== "NA")) {
        complianceItems.push(`Compliance: ${ITEM_LABELS[key]}`);
      } else if (item.value === "Y" && !item.date) {
        complianceItems.push(`Compliance: ${ITEM_LABELS[key]} (missing date)`);
      }
    });
    if (!patient.compliance.lcdEligibility?.checked) {
      complianceItems.push("Compliance: LCD Eligibility");
    }
  }
...
const ChecklistPrintAlertDocument = ({ patientsData }) => {
  const renderAlertSection = (patient) => {
    const alerts = collectPatientAlerts(patient);
    const { incompleteItems, complianceItems, alertRemarks, generalRemarks } = alerts;

    const hasAlerts = incompleteItems.length > 0 || complianceItems.length > 0 || alertRemarks.length > 0 || generalRemarks.length > 0;
    ...
    return (
      <View style={styles.alertSection}>
        ...
        {/* Incomplete Items (Non-Compliance) */}
        ...
        {/* Compliance Items - RED */}
        {complianceItems.length > 0 && (
          <View style={styles.alertSubsection}>
            <Text style={styles.alertSubsectionTitle}>Compliance Issues ({complianceItems.length}):</Text>
            {complianceItems.map((item, idx) => (
              <Text key={idx} style={styles.alertItemCompliance}>• {item}</Text>
            ))}
          </View>
        )}
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.footer}>
          <Text>
            Alert View - Showing only incomplete items, compliance issues, and remarks requiring attention
          </Text>
        </View>
      </Page>
    </Document>
...
export default ChecklistPrintAlertDocument;
