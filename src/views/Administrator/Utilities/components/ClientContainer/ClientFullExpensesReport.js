...
import ClientFullPieChart from "./ClientFullPieChart";

import ClientPieChart from "./ClientPieChart";

class ClientFullExpensesReport extends React.Component {
  render() {
    const {
      patientDashboard,
      numberActive,
            ...
            ? patientDashboard.length &&
              patientDashboard.map((map, index) => (
                <Grid item xs={12} md={5} sm={12} id={index}>
                  ...
                  <div>
                    <ClientFullPieChart series={map.series} />
                  </div>
...
export default ClientFullExpensesReport;
