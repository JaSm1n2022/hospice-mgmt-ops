...
const general = [
  ...
  },
  {
    ...
    noDefault: true,
    isMandatory: true,
    errorMsg: "Pickup Date is required.",
    cols: 2,
  },
  {
    id: "pickupTm",
...
function EquipmentForm(props) {
  ...
  useEffect(() => {
    ...
    console.log("[TEMP]", props.vendorList, temp, props.patientList);
    ...
    if (temp && props.vendorList && props.vendorList.length) {
      const finalFormat =
        [...props.vendorList].filter(
          (f) => f.categoryType?.toLowerCase() === "dme"
        ) || [];
      finalFormat.forEach((f) => {
        f.name = f.name;
        f.value = f.name;
      ...
      vendorInfo = temp.find((t) => t.name === "vendor");
      vendorInfo.options = [DEFAULT_ITEM];
    }
    ...
    if (props.item) {
      console.log("[items]", props.item, patientInfo, vendorInfo);

      const generalFm = { ...props.item };
      generalFm.deliveryTm = moment(new Date(generalFm.deliveryDt)).format(
        "HH:mm"
      );
      ...
      // );
      generalFm.serviceType = [...DME_SERVICE_TYPE].find(
      ...
      const tempArr = [];
      const arr = generalFm.equipmentList.split(",");
      arr.forEach((a, indx) => {
        tempArr.push({
          description: a,
          id: indx,
  ...
  const inputSourceGeneralHandler = ({ target }, source) => {
    ...
    console.log("[Temp Check]", temp.checklist);
    console.log("[Temp Source]", source);
