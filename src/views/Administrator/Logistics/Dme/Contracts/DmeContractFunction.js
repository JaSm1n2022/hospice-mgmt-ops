...
import DmeHandler from "./components/DmeHandler";
...
function DmeContractFunction(props) {
  const classes = useStyles();
  const main = true;
  const [dataSource, setDataSource] = useState([]);
  const [columns, setColumns] = useState(DmeHandler.columns(main));
  ...
  if (
    ...
    const cols = DmeHandler.columns(main).map((col, index) => {
      if (col.name === "actions") {
        return {
          ...col,
          editable: () => false,
  ...
  const createDmeHandler = (payload, mode) => {
    ...
    const params = {
      ...
      location: {
        ...
        code: payload.location.locationCd,
        id: payload.location.id,
        address: payload.location.address,
        contact: payload.location.contactPerson,
      ...
      notes: payload.notes,
      equipment: payload.checklist,
      serviceType: payload.service.name,
    ...
    <>
                ...
                <Card>
                  <CardHeader color="primary">
                    <Grid container justifyContent="space-between">
                      <h4 className={classes.cardTitleWhite}>DME Management</h4>
                    </Grid>
                  </CardHeader>
                  <CardBody>
                    <Grid
      ...
      {isFormModal && (
        <DmeForm
          ...
          dataSource={dataSource}
          createDmeHandler={createDmeHandler}
          mode={mode}
          isOpen={isFormModal}
          isEdit={false}
          item={item}
