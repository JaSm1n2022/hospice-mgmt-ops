  ...
  useEffect(() => {
    if (
      ...
      const record = [...props.transactions.data].filter(
        (p) => p.category.toLowerCase() === "office rent"
      );
      console.log(
        "[record]",
        record[0],
  ...
  return (
    <React.Fragment>
      <Grid justifyContent="space-between" container style={{ padding: 10 }}>
        <Typography variant="h5">Office Rent Report</Typography>
        <Box
          style={{
            padding: 2,
            background: "#ebedeb",
...
export default connect(mapStateToProps, mapDispatchToProps)(OfficeRent);
