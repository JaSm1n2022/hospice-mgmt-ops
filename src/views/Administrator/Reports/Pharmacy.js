  ...
  useEffect(() => {
    if (
      ...
      const record = [...props.transactions.data].filter((p) =>
        ["pharmacy"].includes(p.category.toLowerCase())
      );
  ...
  return (
    <React.Fragment>
      <Grid justifyContent="space-between" container style={{ padding: 10 }}>
        <Typography variant="h5">Pharmacy Expenses Report</Typography>
        <Box
          style={{
            padding: 2,
            background: "#ebedeb",
      ...
      <Grid justifyContent="space-between" container style={{ padding: 10 }}>
        <Typography variant="body1" color="textSecondary">
          Pharmacy reports, including those on prescription medications,
          over-the-counter (OTC) drugs, and pharmacy services.
        </Typography>
      </Grid>
