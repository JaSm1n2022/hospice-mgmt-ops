  ...
  static productItems(details, mode) {
    ...
    for (let item = 0; item < LIMIT_ITEM_PRINT; item++) {
      const record = {
        cnt: item + 1,
        category:
          details[item] && details[item].search
            ? details[item].search.category || details[item].search.category
            : "",
        description:
          details[item] && details[item].search
            ? details[item].search.shortDescription ||
              details[item].search.short_description
            : "",
        units: details[item]
          ? this.unitDistributionHandler(
              details[item].orderQty,
              details[item].search.unitDistribution ||
                details[item].search.unit_distribution ||
                details[item].unit_distribution
            )
          : "",
        size:
          details[item] && details[item].search
            ? details[item].search.size || details[item].search.size
            : "",
        vendor:
          details[item] && details[item].vendor
            ? this.vendorHandler(
                details[item].search.vendor || details[item].search.vendor
              )
            : "",
      };
